...
public class HbaseUtils {
	...
	public static void main(String[] args) throws IOException {
		...
		Goods goods = get(TABLE_NAME, "jd_1361984");
		System.err.println(goods.toString());
		 //HbaseUtils.scan(TABLE_NAME);
	...
	public static Goods get(String tableName, String row) {
		...
		try {
			...
			if (raw.length == 5) {
				goods = new Goods();
				goods.setId(row);
				goods.setDataurl(new String(raw[0].getValue(),"utf-8"));
				goods.setPicpath(new String(raw[1].getValue(),"utf-8"));
				String str = new String(raw[2].getValue(),"utf-8");
				if(str==null||"".equals(str)){
					str="0.00";
				}
				goods.setPrice(Float.parseFloat(str));
				goods.setName(new String(raw[3].getValue(),"utf-8"));
